...
3.Embedding Generation: Convert each chunk into dense vector.

4.Storage in a vector store: Store the vector along with the original chunk+metadata in a vector database.

Retrieval is the real time process of finding the most relevant piece of information from knowledge base.Augmentation refers to the step where the retrieved documents (chunks of relevant context) are combined with the user’s query to form a new, enriched prompt for the LLM.Generation is the final step where a Large Language Model (LLM) uses the user’s query and the retrieved & augmented context to generate a response.
